...
import { DetalhesFamiliasComponent } from "./../detalhes-familias/detalhes-familias.component";
import { ProdutoImagemInterface } from "../../../../@core/interfaces/entities/produto-imagem.interface";

@Component({
    selector: "app-familias-manipulacao",
    templateUrl: "./familias-manipulacao.component.html",
    styleUrls: ["./familias-manipulacao.component.scss"],
})
export class FamiliasManipulacaoComponent implements OnInit {
            ...
            if (this.isEditing) {
                ...
                this.familiasService.editarFamilia(this.editingFamilyId, familiaInput).subscribe(
                    (httpResponseEditarFamilia) => {
                        ...
                        this.familiesStore.updateFamily(this.familiesStore.transformFamilyInterfaceUsingFamiliaInterface(familiaEditada));
                ...
                let productsIds: number[] = this.products.map((product) => product.id);
                familiaInput.codigo = `CAP-${Math.random().toString(32).substr(2).toUpperCase()}`.substring(0, 9);

                this.familiasService.criarFamilia(familiaInput).subscribe(
                    (httpResponseCriarFamilia) => {
                        let familiaCriada: FamiliaInterface = httpResponseCriarFamilia.content;

                        this.serviceAdicionarProdutos(familiaCriada, productsIds).subscribe(
                            () => {
                                familiaCriada.produtos = this.products;
                                this.familiesStore.addFamily(
                                    this.familiesStore.transformFamilyInterfaceUsingFamiliaInterface(familiaCriada)
                                );
                                this.bsModalRef.hide();
                                this.toastrService.success(this.translateService.instant("FAMILY_SUCCESSFULLY_CREATED"));

                                this.bsModalService.show(DetalhesFamiliasComponent, {
                                    initialState: {
                                        familiaCode: familiaCriada.codigo,
                                    },
                                });
